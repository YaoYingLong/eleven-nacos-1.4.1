...
public class HostReactor implements Closeable {
    ...
    public ServiceInfo processServiceJson(String json) {
        ...
        if (oldService != null) { // 一般正常逻辑oldService不为null
            if (oldService.getLastRefTime() > serviceInfo.getLastRefTime()) { // 旧的服务修改时间大于新获取到的服务修改时间
                NAMING_LOGGER.warn("out of date data received, old-t: " + oldService.getLastRefTime() + ", new-t: " + serviceInfo.getLastRefTime());
            }
            ...
            if (newHosts.size() > 0) { // 存在新增实例将changed置为true，输出相应日志
                changed = true;
                NAMING_LOGGER.info("new ips(" + newHosts.size() + ") service: " + serviceInfo.getKey() + " -> " + JacksonUtils.toJson(newHosts));
            }
            if (remvHosts.size() > 0) { // 存在删除实例将changed置为true，输出相应日志
                changed = true;
                NAMING_LOGGER.info("removed ips(" + remvHosts.size() + ") service: " + serviceInfo.getKey() + " -> " + JacksonUtils.toJson(remvHosts));
            }
