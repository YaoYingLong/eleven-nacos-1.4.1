...
public class LocalConfigInfoProcessor {
    ...
    public static String getFailover(String serverName, String dataId, String group, String tenant) {
        ...
        if (!localPath.exists() || !localPath.isFile()) {
            return null; // 若本地文件不存在则直接返回空
        }
        try {
            return readFile(localPath); // 从磁盘中读取文件内容
    ...
    public static void saveSnapshot(String envName, String dataId, String group, String tenant, String config) {
        if (!SnapShotSwitch.getIsSnapShot()) { // 默认是支持本地缓存的
            return;
        }
        File file = getSnapshotFile(envName, dataId, group, tenant);
        if (null == config) {
            try {
                IoUtils.delete(file); // 若获取到的结果为null，则删除本地缓存文件
            } catch (IOException ioe) {
            ...
            try {
                ...
                if (JvmUtil.isMultiInstance()) { // 默认为false
                    ConcurrentDiskUtil.writeFileContent(file, config, Constants.ENCODE); // 写数据到文件中
                } else {
                    IoUtils.writeStringToFile(file, config, Constants.ENCODE); // 写数据到文件中
                }
            } catch (IOException ioe) {
    ...
    static {
        LOCAL_FILEROOT_PATH = System.getProperty("JM.LOG.PATH", System.getProperty("user.home")) + File.separator + "nacos" + File.separator + "config";
