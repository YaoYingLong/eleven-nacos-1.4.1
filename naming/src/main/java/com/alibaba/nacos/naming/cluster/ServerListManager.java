...
public class ServerListManager extends MemberChangeListener {
    ...
    private class ServerInfoUpdater implements Runnable {
        ...
        @Override
        public void run() { // 通过轮训的方式每2秒去其他服务端成员去查询一下成员状态
            List<Member> members = servers;
            if (members.isEmpty()) {
                return; // 若服务端成员为空则直接返回
            }
        ...
        public void run() {
            try {
                ...
                if (!contains(EnvUtil.getLocalAddress())) {
                    Loggers.SRV_LOG.error("local ip is not in serverlist, ip: {}, serverlist: {}", EnvUtil.getLocalAddress(), allServers);
                ...
                if (allServers.size() > 0 && !EnvUtil.getLocalAddress().contains(IPUtil.localHostIP())) {
                    for (Member server : allServers) { // 每2s将当前服务端机器状态同步给其他服务端机器
                        ...
                        msg.setData(status);
                        synchronizer.send(server.getAddress(), msg); // ServerStatusSynchronizer异步发送HTTP请求
                    }
                }
            } catch (Exception e) {
                Loggers.SRV_LOG.error("[SERVER-STATUS] Exception while sending server status", e);
            } finally { // 再将当前任务放入延时线程池中，默认2s执行一次
