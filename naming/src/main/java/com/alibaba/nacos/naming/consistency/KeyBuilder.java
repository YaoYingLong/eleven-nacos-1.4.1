...
public class KeyBuilder {
    ...
    public static String getNamespace(String key) {
        if (matchSwitchKey(key)) { // 若key以00-00---000-NACOS_SWITCH_DOMAIN-000---00-00结尾
            return StringUtils.EMPTY;
        }
        if (matchServiceMetaKey(key)) {  // key以com.alibaba.nacos.naming.domains.meta.或meta.开头
            return key.split(NAMESPACE_KEY_CONNECTOR)[0].substring(SERVICE_META_KEY_PREFIX.length());
        }
        if (matchEphemeralInstanceListKey(key)) {  // key以com.alibaba.nacos.naming.iplist.ephemeral.开头
            return key.split(NAMESPACE_KEY_CONNECTOR)[0].substring(INSTANCE_LIST_KEY_PREFIX.length() + EPHEMERAL_KEY_PREFIX.length());
        }
        if (matchInstanceListKey(key)) {   // key以com.alibaba.nacos.naming.iplist.或iplist.开头
